...
// تهيئة النظام
class SeaStarSystem {
    // تعريف المتغيرات الأساسية للكلاس هنا لضمان وجودها
    constructor() {
        this.synth = null;
        this.audioContext = null;
        this.achievements = {}; // يتم تهيئتها في initGamification
        this.userPoints = 0; // يتم تهيئتها في initGamification
        this.analyticsData = {}; // يتم تهيئتها في initAnalytics
        this.notificationQueue = []; // يتم تهيئتها في initNotifications
        this.isShowingNotification = false; // يتم تهيئتها في initNotifications
        this.loadingScreen = null; // يتم تهيئتها في initLoading
        this.neuralNetwork = null; // يتم تهيئتها في initNeuralNetwork
        this.blockchain = null; // يتم تهيئتها في initBlockchain
        this.quantumStates = []; // يتم تهيئتها في initQuantumEffects
        this.init();
    }
    ...
    initCursor() {
        ...
        interactiveElements.forEach(element => {
            ...
            element.addEventListener('mouseleave', () => {
                ...
                // تم تعديل اللون الافتراضي (يفترض تعريفه في CSS أو إزالته إذا لم يكن موجودًا)
    ...
    initParticles() {
        // التحقق من وجود مكتبة particlesJS
    ...
    initTheme() {
        ...
        if (themes[savedTheme]) { // التحقق من وجود الثيم
            this.applyTheme(themes[savedTheme]);
        } else {
             this.applyTheme(themes['cyber']);
        }
    ...
    createThemeSelector(themes) {
        ...
        Object.keys(themes).forEach(themeName => {
            ...
            btn.addEventListener('click', () => {
                ...
                // إطلاق حدث تغيير الثيم لنظام Gamification
                document.dispatchEvent(new Event('themeChanged')); 
            });
            
            document.body.appendChild(container); // تم نقل الإضافة هنا لضمان وجود العنصر
    ...
    initVoiceCommands() {
        ...
        if (typeof annyang !== 'undefined') {
            const commands = {
                ...
                // أمر المساعدة
                'مساعدة': () => this.showNotification('الأوامر المتاحة: انتقل إلى الرئيسية، غير الثيم، تشغيل الموسيقى، إظهار الإحصائيات.', 'info')
            };
    ...
    createVoiceControlButton() {
        if (typeof annyang === 'undefined') return; // تأكد من وجود annyang
        ...
        // إضافة رد فعل بعد تنفيذ الأمر
        annyang.addCallback('resultMatch', (userSaid, commandText, phrases) => {
            this.showVoiceFeedback(`تم تنفيذ: ${commandText.split('(')[0]}`); // عرض الأمر الذي تم تنفيذه
            setTimeout(() => {
                this.hideVoiceFeedback();
            }, 2000);
        });

        // رسالة خطأ عند عدم التعرف على الأمر
        annyang.addCallback('resultNoMatch', (phrases) => {
            this.showVoiceFeedback(`لم أتمكن من فهم: "${phrases[0]}"`);
            setTimeout(() => {
                this.hideVoiceFeedback();
            }, 3000);
        });
    ...
    initAudio() {
        // التحقق من وجود مكتبة Tone
        if (typeof Tone !== 'undefined') {
            // إنشاء سياق الصوت
            this.audioContext = new Tone.Context();
            
            // تهيئة مركب الصوت (Synth)
            ...
            // مؤثرات صوتية للتمرير (تم إزالة initScrollSounds لعدم وجود تعريف لها)
        }
    }
    ...
    playMusic() {
        if (this.synth) {
            ...
            this.showNotification('الموسيقى قيد التشغيل (C4)', 'info');
        } else {
            this.showNotification('خطأ: نظام الصوت غير مهيأ', 'error');
        }
    }

    pauseMusic() {
        if (this.synth && this.synth.envelope.value > 0) { // التحقق من وجود صوت يعمل
            // لا يوجد طريقة 'triggerRelease' مباشرة لإيقاف Synth،
            // يمكن استخدام إيقاف الصوت النشط إذا كان مسجلاً، أو استخدام غيتار (DuoSynth)
            // أو ببساطة عرض رسالة للمحاكاة
            this.showNotification('تم إيقاف الموسيقى (محاكاة)', 'info');
        } else if (this.synth) {
            this.showNotification('الموسيقى متوقفة بالفعل', 'info');
        }
    }
    ...
    setVolume(level) {
        ...
        if (this.synth) {
            // Tone.js يستخدم ديسيبل، و0.5 في شريط التمرير يجب أن يعادل -6dB تقريبًا
            const db = parseFloat(level) * 40 - 40; // تحويل من 0-1 إلى -40dB إلى 0dB
    ...
    // (تم إزالة initScrollSounds لعدم وجود تعريف لها)
    playScrollSound() {
        if (typeof Tone !== 'undefined') {
    ...
    initAnimations() {
        // التحقق من وجود مكتبة GSAP
        if (typeof gsap === 'undefined') return;

        // تهيئة GSAP
        // التأكد من أن الإضافات (plugins) متاحة قبل التسجيل
        if (typeof ScrollTrigger !== 'undefined' && typeof TextPlugin !== 'undefined') {
             gsap.registerPlugin(ScrollTrigger, TextPlugin);
        } else if (typeof ScrollTrigger !== 'undefined') {
             gsap.registerPlugin(ScrollTrigger);
        } else if (typeof TextPlugin !== 'undefined') {
        ...
        // تأثيرات 3D (تمت إزالة init3DAnimations لعدم وجود تعريف لها)
        
        // تأثيرات الحروف (تمت إزالة initLetterAnimations لعدم وجود تعريف لها)
    }
    ...
    initTextAnimations() {
        ...
        gsap.utils.toArray('.morph-text').forEach(element => {
            const texts = element.dataset.texts?.split('|') || []; // استخدام عامل التشغيل الاختياري
            let index = 0;
    ...
    initGamification() {
        ...
        this.userPoints = parseInt(localStorage.getItem('seastar-points') || '0', 10);
    ...
    checkAchievements() {
        ...
        // تغيير الثيم
        const themeChanges = parseInt(localStorage.getItem('seastar-theme-changes') || '0', 10);
    ...
    showAchievementBadge(title, description, points) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                badge.remove(); // استخدام remove بدلاً من document.body.removeChild
            }, 500);
        }, 3000);
    }
    ...
    trackUserActions() {
        ...
        window.addEventListener('scroll', () => {
            ...
            if (scrollHeight > 0) { // تجنب القسمة على صفر
                ...
                if (maxScroll >= 90 && !this.achievements.scrollMaster?.unlocked) {
        ...
        document.addEventListener('themeChanged', () => {
            let changes = parseInt(localStorage.getItem('seastar-theme-changes') || '0', 10);
            changes++;
    ...
    collectAnalytics() {
        // عدد المشاهدات
        this.analyticsData.pageViews = parseInt(localStorage.getItem('seastar-page-views') || '0', 10);
        ...
        window.addEventListener('beforeunload', () => {
            ...
            // التحقق من وجود القيمة المحفوظة قبل الإضافة
            const savedTime = parseInt(localStorage.getItem('seastar-time-spent') || '0', 10);
            this.analyticsData.timeSpent = savedTime + timeSpent;
    ...
    createAnalyticsPanel() {
        ...
        document.body.appendChild(panel); // إضافة اللوحة إلى DOM

        // تحديث البيانات
        setInterval(() => {
            const timeInSeconds = Math.floor(this.analyticsData.timeSpent / 1000) + Math.floor((Date.now() - performance.timing.navigationStart) / 1000); // تحديث الوقت بناءً على بداية التنقل
            ...
            document.getElementById('timeSpent').textContent = timeInSeconds;
    ...
    initTouchGestures() {
        ...
        if (typeof Hammer !== 'undefined') {
            ...
            // وظيفة مساعدة لاستخراج قيمة المقياس
            ...
            hammer.on('pinchin', () => {
                const newScale = Math.max(0.5, getCurrentScale() - 0.1);
                document.body.style.transform = `scale(${newScale})`;
            });
            
            hammer.on('pinchout', () => {
                const newScale = Math.min(2, getCurrentScale() + 0.1);
                document.body.style.transform = `scale(${newScale})`;
            });
        }
    }
    ...
    showNotification(message, type = 'info') {
        ...
        const icon = `<i class="fas fa-${iconClass}"></i>`;
        ...
        // استخدام querySelector للحصول على مركز الإشعارات
    ...
    simulateLoading() {
        ...
        const interval = setInterval(() => {
            ...
            // تحديث النصوص
            if (progress > 33 && stats[0]) stats[0].classList.add('complete');
            if (progress > 66 && stats[1]) stats[1].classList.add('complete');
            if (progress > 99 && stats[2]) stats[2].classList.add('complete');
    ...
    initDynamicColors() {
        ...
        setInterval(() => {
            const hour = new Date().getHours();
            document.documentElement.style.setProperty('--time-hour', hour.toString());
    ...
    initScrollEffects() {
        // شريط التقدم
        const progressBarContainer = document.createElement('div'); // تغيير اسم المتغير لتجنب التعارض
        progressBarContainer.className = 'progress-container';
        progressBarContainer.innerHTML = '<div class="progress-bar global"></div>'; // إضافة كلاس global لتمييزه عن أشرطة التقدم الأخرى
        ...
        window.addEventListener('scroll', () => {
            ...
            if (height > 0) { // تجنب القسمة على صفر
                const scrolled = (winScroll / height) * 100;
                document.querySelector('.progress-bar.global')?.style.width = `${scrolled}%`;
            }
        });
    ...
    createHUD(id, label) {
        ...
        // تعديل ترتيب العناصر لتبدو متراصة
        const index = ['time', 'date', 'fps'].indexOf(id);
        hud.style.top = `${50 + (index * 80)}px`;
    ...
    // 20. تأثيرات الكتابة
    initTypingEffects() {
        document.querySelectorAll('.typing-text:not(.loading-text)').forEach(element => {
            const text = element.textContent;
            element.textContent = '';
            ...
            star.addEventListener('click', () => {
                ...
                stars.forEach((s, i) => {
                    if (i <= index) {
                        ...
                        // لا حاجة لإزالة/إضافة نفس الكلاس، فقط ضمان الإضافة
                        s.classList.remove('far'); // إزالة نجمة فارغة
                        s.classList.add('fas'); // إضافة نجمة ممتلئة
                    } else {
                        s.classList.remove('active');
                        s.classList.add('far'); // إضافة نجمة فارغة
                        s.classList.remove('fas'); // إزالة نجمة ممتلئة
                    }
                });
                
                // حفظ التقييم
                localStorage.setItem('seastar-rating', rating.toString());
    ...
    performanceMonitor() {
        const monitor = () => {
            // التحقق من وجود performance.memory
        ...
        window.addEventListener('scroll', () => {
            if (!ticking) {
                requestAnimationFrame(() => {
                    // تم إزالة استدعاء this.onScroll() لعدم وجود تعريف لها
                    ticking = false;
                });
                ticking = true;
            }
    ...
    detectDevice() {
        ...
        if (isMobile) {
            ...
            // تقليل عدد الجسيمات (متغير CSS لـ particlesJS)
    ...
    initSentimentAnalysis() {
        // تحليل مشاعر النص
        document.addEventListener('click', (e) => {
            // استخدام matches لضمان وجود العنصر المستهدف
            if (e.target instanceof Element && e.target.matches('.user-content')) {
    ...
    initSmartRecommendations() {
        ...
        // تم إزالة تعريف recommendations لعدم استخدامه مباشرة
    ...
    initWeatherIntegration() {
        // إنشاء عنصر الطقس لضمان وجوده
        const weatherElement = document.createElement('div');
        weatherElement.className = 'weather-display hud-element';
        weatherElement.style.position = 'fixed';
        weatherElement.style.top = '260px';
        weatherElement.style.right = '30px';
        ...
        updateWeather();
        setInterval(updateWeather, 60000); // كل دقيقة
    }

    updateWeather(temp, condition) {
        const weatherElement = document.querySelector('.weather-display');
        if (weatherElement) {
            weatherElement.innerHTML = `<span class="hud-border"></span> ${condition} ${Math.round(temp)}°C`;
        }
    }
    ...
    loadPreferences() {
        ...
        // تطبيق التفضيلات
        if (prefs.theme) {
             const themes = { // تم تضمين تعريف الثيمات هنا لتطبيقها
            ...
            if (themes[prefs.theme]) {
                this.applyTheme(themes[prefs.theme]);
            }
        }
    ...
    savePreferences() {
        ...
        window.addEventListener('beforeunload', () => {
            ...
            const prefs = {
                ...
                volume: volumeControl ? volumeControl.value : 0.5,
    ...
    initPWA() {
        ...
        // تم إزالة تعريف deferredPrompt هنا لضمان استخدام المتغير الشامل في نهاية الكود
        
        // استخدام deferredPrompt المعرف خارج الكلاس
    ...
    showInstallButton() {
        ...
        installBtn.onclick = window.installApp; // استخدام الدالة المعرفة خارج الكلاس
    ...
    initBlockchain() {
        ...
        document.addEventListener('click', (e) => {
            if (e.target instanceof Element && e.target.matches('.important-event')) {
    ...
    triggerQuantumEffect(effect) {
        ...
        switch(effect) {
            case 'superposition':
                ...
                this.showNotification('تأثير الكوانتوم: التراكب (Superposition)', 'info');
                break;
            ...
            case 'entanglement':
                ...
                this.showNotification('تأثير الكوانتوم: التشابك (Entanglement)', 'info');
                break;
            ...
            case 'tunneling':
                ...
                this.showNotification('تأثير الكوانتوم: النفق (Tunneling)', 'info');
                break;
        }
    }
    ...
    scrollToSection(section) {
        ...
        if (section === 'home') {
            ...
            let nextSection = null; // تهيئة بـ null
            ...
            target = nextSection ? nextSection.offsetTop : current;
        } else if (section === 'previous') {
            const sections = document.querySelectorAll('section');
            const current = window.scrollY;
            let prevSection = null; // تهيئة بـ null
            ...
            target = prevSection ? prevSection.offsetTop : 0;
        } else {
    ...
    cycleTheme() {
        ...
        document.dispatchEvent(new Event('themeChanged')); // إطلاق حدث تغيير الثيم
    }
    ...
    toggleTheme() {
        const current = localStorage.getItem('seastar-theme') || 'cyber';
        // التبديل بين وضعين رئيسيين ( matrix و ocean )
...
function showInstallPromotion() {
    // التحقق من عدم وجود زر التثبيت مسبقاً
    if (document.querySelector('.install-promotion')) return;
...
function installApp() {
    if (deferredPrompt) {
        ...
        deferredPrompt.userChoice.then((choiceResult) => {
            ...
            deferredPrompt = null;
            document.querySelector('.install-promotion')?.remove();
        });
    }
}
